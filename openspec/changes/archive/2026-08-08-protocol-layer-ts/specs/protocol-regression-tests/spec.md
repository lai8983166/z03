# Spec Delta: protocol-regression-tests

> 本 change 新增协议层回归测试套件 + JSDoc 类型注解，锁住已联调通过的协议解析行为，为 change 3-5 的真 .ts 迁移提供安全网。文件保持 `.js`，行为绝对不变。
...
- **THEN** 不调用 `document.getElementById` 等 DOM API；DOM 相关方法（`readCell`/`updateAllToTable` 等）不在本 change 测试范围

### Requirement: 协议层 JSDoc 类型注解
`TcpBridge.js`、`js/Udp.js`、`js/CommandBuilder.js`、`js/BinaryTableHelper.js` MUST 添加 JSDoc 类型注解（`@param`/`@returns`/`@typedef`/`@type`），使 `tsc --noEmit`（change 1 的 `allowJs` 配置）能识别字段、参数、返回值类型。
...
4 个协议层源文件的可执行语句 MUST 逐字不动——本 change 只允许增加注释与 JSDoc 注解（`@param`/`@returns`/`@typedef`/`@type` 等），MUST NOT 改变任何变量赋值、表达式、控制流、字节布局、命令表数值。
