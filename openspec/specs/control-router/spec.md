...
> control 模块（消息路由）：将 server.ts 中 `handleJsonControlMessage`/`handleControlCommand` 抽出为独立模块。本 capability 在 control-module change 中建立，并修复 3b-3a/3b-3b 引入的参数名冲突 bug。
